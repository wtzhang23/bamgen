...
impl Construct {
    fn run(self) {
        ...
        let mut indices = rand::seq::index::sample(&mut rng, self.read_count - 2, self.n_ref - 1)
            .into_iter()
            .map(|x| x + 1)
            .collect::<Vec<usize>>();
        indices.sort();
        ...
        for (ri, (reference_len, read_count)) in
            lengths.into_iter().zip(counts.into_iter()).enumerate()
        {
            ...
            let mut indices = rand::seq::index::sample(
                &mut rng,
                reference_len - self.min_read_len,
                batches.len(),
            )
            .into_vec();
            indices.sort();
            ...
            if has_umi {
                ...
                for (batch_size, batch_pos) in batches.into_iter().zip(indices) {
                    ...
                    for _r in 0..batch_size {
                        // generate batch_size # of reads
                        let read_len = rng.gen_range(
                            self.min_read_len,
                            std::cmp::min(reference_len - batch_pos, self.max_read_len),
                        );
                        ...
                        for (umi, count) in umis.drain() {
                            for _i in 0..count {
                                ...
                                if self.paired {
                                    // for pair
                                    let pair_pos =
                                        rng.gen_range(0, reference_len - self.min_read_len);
                                    let len = rng.gen_range(
                                        self.min_read_len,
                                        std::cmp::min(reference_len - pair_pos, self.max_read_len),
                                    );
        ...
        file.write_all(ron::to_string(&ground_truth).unwrap().as_bytes())
            .unwrap();
...
impl Diff {
    fn run(self) {
        ...
        let mut group_ids = HashMap::<Vec<u8>, Option<i64>>::new();
            ...
            if self.paired {
                if let Some(group_id) = record.aux(self.group_tag.as_bytes()) {
                    ...
                    if let Some(other_id) = group_ids.get(record.qname()) {
                        let other_id = other_id.expect("Read w/ same qname had no group id beforehand while this read has one.\n");
                        assert_eq!(
                            other_id, group_id,
                            "Failed to transfer group id to paired read."
                        );
                    } else {
                        group_ids.insert(record.qname().to_owned(), Some(group_id));
                    }
                } else {
                    let prev = group_ids.insert(record.qname().to_owned(), None);
                    if let Some(id_opt) = prev {
                        assert!(id_opt.is_none(), "Read w/ same qname had a group id.\n");
                    }
                }
            }
        }
        assert_eq!(
            count, ground_truth.total_reads,
            "Deduplication left out/added too much reads."
        );

        // check that qnames match counts
        for (qname, count) in num_per_qname.drain() {
            let true_qname_count = ground_truth.reads_per_qname.remove(&qname).expect(&format!(
                "qname {} does not exist in ground truth",
                &String::from_utf8(qname.clone()).unwrap()
            ));
            assert_eq!(
                count,
                true_qname_count,
                "qname {} lost some or gained some reads",
                &String::from_utf8(qname).unwrap()
            );
        }
        ...
        for (loci, count) in predicted_cpl.drain() {
            ...
            let true_count = ground_truth
                .cpl_prior_duplication
                .remove(&loci)
                .expect(&format!("Loci {:#?} not found.", loci));
            let diff = true_count as i64 - adj_count as i64;
            diff_data.push((adj_count, true_count, diff));
        }
        assert!(ground_truth.cpl_prior_duplication.is_empty());

        let file = File::create(&self.diff).unwrap();
        let mut file = LineWriter::new(file);
        file.write_all(format!("{}\t{}\t{}\n", "output", "ground truth", "diff").as_bytes())
            .unwrap();

        for (count, gt_count, diff) in diff_data.into_iter() {
            file.write_all(format!("{}\t{}\t{}\n", count, gt_count, diff).as_bytes())
                .unwrap();
        }
    }
}
